...
Hệ thống SHALL cho phép member xem hồ sơ hiện tại của chính mình thông qua select menu trên member management panel, bao gồm `ingame_name`, `game_id`, `mon_phai`, và trạng thái/ảnh QR ngân hàng của chính user.
...
- **THEN** hệ thống SHALL trả về ephemeral message hiển thị `ingame_name`, `game_id`, và `mon_phai` hiện tại của user
- **AND** nếu profile có QR ngân hàng thì phản hồi SHALL hiển thị QR đó cho chính user
...
Hệ thống SHALL cho phép member cập nhật hồ sơ qua flow panel gồm hiển thị hồ sơ cũ, chọn `mon_phai` từ danh sách cố định, rồi nhập `ingame_name` và `game_id` bằng modal.
...
#### Scenario: Member selects mon phai before entering name and game ID
- **WHEN** user chọn một `mon_phai` hợp lệ từ select menu cập nhật hồ sơ
- **THEN** hệ thống SHALL mở modal nhập `ingame_name` và `game_id`
- **AND** nếu user đã có profile thì modal SHALL prefill `ingame_name` và `game_id` hiện tại khi Discord component hỗ trợ

#### Scenario: Member submits valid profile modal
- **WHEN** user submit modal cập nhật hồ sơ với `ingame_name` không rỗng, `game_id` không rỗng và unique trong guild, và `mon_phai` đã chọn hợp lệ
...
#### Scenario: Member submits duplicate game ID
- **WHEN** user submit modal cập nhật hồ sơ với `game_id` đang được `user_id` khác sử dụng trong cùng guild
- **THEN** hệ thống SHALL từ chối lưu profile
- **AND** phản hồi SHALL là ephemeral cho biết `game_id` đã tồn tại trong guild
...
- **THEN** hệ thống SHALL trả về ephemeral message yêu cầu user tạo hồ sơ bằng `Cập nhật hồ sơ` trước khi upload QR
