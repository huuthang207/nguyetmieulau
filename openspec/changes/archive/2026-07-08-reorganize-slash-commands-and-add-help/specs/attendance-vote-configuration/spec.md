...
- **WHEN** user có quyền `Administrator` hoặc có `admin role` dùng `/config channel` với một channel hợp lệ
...
- **WHEN** user có quyền `Administrator` hoặc có `admin role` dùng `/config member-role` với một role hợp lệ
...
- **WHEN** user có quyền `Administrator` hoặc có `admin role` dùng `/config admin-role` với một role hợp lệ
...
- **WHEN** user không có quyền `Administrator` và không có `admin role` dùng một subcommand của `/config`
...
- **WHEN** admin dùng `/vote create` nhưng guild chưa có một hoặc nhiều cấu hình cần thiết trong `attendance channel`, `member role`, `admin role`
